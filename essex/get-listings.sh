...
cat $fname | jq -r '.[] | select((.beds|tonumber) >= '$min_br') | select((.baths|tonumber) >= 2) | .floorplans[].units[] | .unit_code + ":" + .beds + ":" + .available_date + ":" + .square_feet' | while IFS=: read -r unit_code beds avail_date sqft; do
    ...
    cat $fname | jq -r ".pricing|select(.!=null)|.[] | select((.Term|tonumber) >= $min_lease) | select((.Rent|tonumber) > $rent_range >= (.Rent|tonumber)) | .Term + \":\" + .Rent" | while IFS=: read -r term rent; do
